  ...
  if (view === 'closed') {
    return (
      <button
        ...
        className="flex items-center gap-1.5 text-sm text-[var(--text-secondary)] hover:text-[var(--foreground)]"
      >
        <MenuIcon />
        Menu
      </button>
          ...
          {pending.length === 0 ? (
            ...
            <ul className="mt-2 space-y-1.5">
              {pending.map((t, i) => (
                ...
                >
                  <span className="flex-1 truncate">{pendingNames[i]}</span>
                  <button
                    type="button"
          ...
          {trackedTeamOptions.length > 0 ? (
            <>
              ...
              <p className="mt-2 text-xs text-[var(--text-muted)]">
                Recolors the app to match and puts this team&rsquo;s logo next
                to the header. It also updates your Home Screen icon — remove
                and re-add the app in Safari to see that change (iOS caches
                icons per device).
              </p>
            </>
          ) : (
